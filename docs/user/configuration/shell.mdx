...
- [Desktop frame and screen-edge panels](#desktop-frame-and-screen-edge-panels)
...
[shell.animation]
enabled = true
speed   = 1.0   # 1.0 = normal; the Basic slider's 0 position writes enabled = false
...
floating_layer         = "overlay" # overlay | top; applies to floating and screen-edge panels
launcher_placement       = "floating" # attached | floating | screen_edge
clipboard_placement      = "floating" # attached | floating | screen_edge
control_center_placement = "attached" # attached | floating | screen_edge
wallpaper_placement      = "attached" # attached | floating | screen_edge
session_placement        = "attached" # attached | floating | screen_edge
polkit_placement         = "floating" # attached | floating | screen_edge
launcher_position        = "center"   # auto | center | top_left | … (floating or screen_edge)
clipboard_position       = "center"   # auto | center | top_left | … (floating or screen_edge)
polkit_position          = "center"   # auto | center | top_left | … (floating or screen_edge)
...
- `shell.animation.enabled` disables all animated transitions globally. In Basic settings, animation speed `0` is an
  **Off** position backed by `enabled = false`; the last valid positive `speed` remains stored, and selecting a positive
  value atomically re-enables animations. Persisted `speed` values remain in the `0.1`–`4.0` range.
...
- `shell.panel.floating_layer` selects the layer-shell layer for floating and screen-edge Launcher, Clipboard, Control Center, Wallpaper, Session, and Setup Wizard panels. `overlay` is the default and keeps these panels above fullscreen content. Use `top` when a compositor places input-method candidate popups below the Overlay layer. Bar-attached panels continue to use their bar's configured layer; system overlays such as the polkit prompt keep their own layer policy. Plugin panels expose a separate per-entry **Layer** setting under **Settings → Plugins**. Change the built-in panel setting in **Settings → Panels → General**.
- `shell.panel.launcher_placement`, `clipboard_placement`, `control_center_placement`, `wallpaper_placement`, `session_placement`, and `polkit_placement` accept `attached`, `floating`, or `screen_edge`. An attached panel keeps its own layer surface and anchors it to the host bar's inward edge; floating placement opens it detached. `screen_edge` joins a chosen screen edge without requiring a bar. Defaults are floating at the center position for Launcher, Clipboard, and the polkit auth prompt; Control Center, Wallpaper, and Session default to attached.
- Per-panel `*_position` keys (`launcher_position`, `clipboard_position`, `control_center_position`, `wallpaper_position`, `session_position`, `polkit_position`) apply when placement is `floating` or `screen_edge`. For floating panels, `auto` keeps the panel bar-relative; `center` and the edge/corner tokens anchor on screen. For `screen_edge`, the token selects the edge and alignment as described below.
- `shell.panel.floating_offset` sets the gap in pixels a floating panel keeps from the bar edge, or from the screen edge when it is pinned with a `*_position` (default `8`). It also spaces panels using the reserved edge area when multiple bars share an edge. `attached` and `screen_edge` panels ignore it.
...
## Desktop frame and screen-edge panels

The optional frame draws a continuous palette-colored perimeter around each output. Its inner opening reveals the wallpaper, with rounded or chamfered corners. It uses a click-through Bottom-layer surface behind windows and remains available when Noctalia's wallpaper is disabled or an external Background-layer wallpaper is used. All settings are public geometry and color roles; rendering does not depend on a preset name.

```toml
[shell.desktop_frame]
enabled = true                  # default: false
fill = "surface"                # semantic palette role or ColorSpec
border = "outline"
border_width = 0
left = 16                       # logical px of frame on each output edge
top = 16
right = 16
bottom = 48
radius = 32                     # inner opening radius

[shell.panel]
launcher_placement = "screen_edge"
launcher_position = "bottom_center"
```

`screen_edge` uses the panel's `*_position` to choose `top_left`, `top_center`, `top_right`, `center_left`, `center_right`, `bottom_left`, `bottom_center`, or `bottom_right`. It aligns to the frame's inner edge and uses the frame fill when enabled; without a frame it joins the physical screen edge. Existing `auto` or `center` values resolve to `bottom_center` in this placement. The same placement is available to plugin panels.

The frame also supports `chamfered` with four `chamfer_*` dimensions, four named `*_shelf` tables, up to eight `shelf_1`–`shelf_8` tables with an `edge`, and three `border_layer_1`–`border_layer_3` tables. Shelves add joined protrusions to the perimeter. A positive `reference_width` and `reference_height` pair scales the geometry proportionally on other output sizes; zero uses logical pixels directly. The frame updates with output size, palette, and settings previews.
...
[osd.activity]
presentation = "standalone" # standalone | section | attached
bar = ""                    # stable [bar.<name>] name; empty matches any bar
section = ""                # stable [[bar.<name>.section]] id
output = "focused"          # focused | all | output connector/selector
placement = "auto"          # auto | before | after | below | above | left | right
motion = "inherit"          # inherit | off
material = "inherit"        # inherit | surface | transparent
...
`[osd.activity]` routes volume, brightness, and notification activity without tying the renderer to a theme or
profile name. The default `standalone` presentation preserves the normal OSD and notification toast surfaces on every
theme. `section` temporarily replaces a configured bar section. `attached` presents the activity beside that section.
If the selected bar, section, or output is unavailable, Noctalia uses the standalone surface so the event remains
visible.

`bar` is the stable name from `[bar.<name>]`, and `section` is the stable `id` from a
`[[bar.<name>.section]]` entry. `output = "focused"` follows the interactive output, `"all"` presents on every matching
bar, and a connector or output selector targets matching outputs. `placement = "auto"` chooses the side facing into the
screen; the other values place attached activity relative to its section. `motion = "inherit"` follows the shared
animation policy, while `"off"` updates immediately. `material = "inherit"` uses the shared UI material; `"surface"`
forces a filled surface and `"transparent"` removes the activity fill.

Embedded volume and brightness activity keeps its live slider, including pointer and wheel adjustment. Clicking its
card opens the matching Audio or Brightness controls. For notifications, left-click invokes the default action when one
exists and otherwise opens notification history; right-click dismisses it. Hovering any routed activity pauses its
transient timeout, and leaving resumes the remaining time. Section presentation uses the named section's existing
bounds rather than opening a separate hover panel.

Each supported kind can override any part of the base route. Empty fields inherit the base value:
...
[osd.activity.brightness]
presentation = ""

[osd.activity.notification]
presentation = ""
```

For a neutral, terminal-oriented layout, define a stable status section and attach immediate activity below it. The
activity still inherits the same material and colors as the rest of the shell:

```toml
[[bar.default.section]]
id = "terminal-status"
widgets = ["network", "volume", "notifications"]
anchor = "end"
alignment = "end"
