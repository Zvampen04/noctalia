...
When `[osd.activity]` routes notifications into a bar section or attached activity card, the same DND and filter result
still decides whether the notification is eligible. The shared presenter shows one activity at a time: critical
notifications preempt volume/brightness activity, which preempts normal and low-urgency notifications. Hover pauses the
remaining timeout. Left-click invokes the default action when present and otherwise opens notification history;
right-click dismisses it. If the configured bar or section is unavailable, the notification uses the standalone
fallback without leaving a duplicate embedded card behind.
