...
**Placement** matches built-in shell panels: `attached` anchors to a bar, `floating` opens detached, and `screen_edge`
joins a chosen screen edge without a bar. The host
...
panels: a placement selector, **Position** and **Layer** dropdowns (for floating or screen-edge panels), and
**Open Near Click**.

| `[[panel]]` manifest key | `plugin_settings` key | Notes |
|--------------------------|-------------------------|-------|
| `placement` | `<entry>_placement` | `attached`, `floating`, or `screen_edge` (default `floating`) |
| `position` | `<entry>_position` | `auto`, `center`, or a screen anchor; applies when floating or screen-edge |
...
| `layer` | `<entry>_layer` | `top` (default) or `overlay`; applies when floating or screen-edge (optional manifest default requires <PluginApiBadge feature="panel-layer" />) |
...
follow the same rules as built-in panels (pinned screen positions and screen-edge placement disable open-near-click).
For screen-edge placement, use one of the eight edge/corner anchors; `auto` and `center` resolve to `bottom_center`.

**Layer** controls the floating or screen-edge surface's layer-shell layer. `top` keeps the panel under fullscreen windows; `overlay`
