...
Home shows its compact media card according to `control_center.media.home_visibility`. In automatic mode, an enabled
bar media widget suppresses the duplicate Home card; disabling or removing that bar widget lets the Home card return.
Shortcut actions and their right-click destinations are listed in the shortcuts guide. Unsupported capabilities stay
disabled or absent instead of presenting a control that cannot change state—for example, Monitor has no write control
without a writable brightness device.
...
On the compact Home calendar, wheel anywhere across the full week strip to move one week at a time. Click a day number
to open the month view with that date selected; the selected month follows the clicked date even when the week crossed a
month boundary. In the month view, wheel over the calendar card to move one month, click a day or its event dots to
select it, and click the date heading to return to today. Event dots and the event list use the same shared calendar
snapshot, including vdir changes detected while the panel is closed.
